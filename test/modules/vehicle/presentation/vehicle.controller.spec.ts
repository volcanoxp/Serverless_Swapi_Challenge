...
import { BadRequest, InternalServerError, NotFoundRequest, SuccessRequest } from '../../../../src/shared/responses'
...
describe('VehicleController', () => {
  ...
  let vehicleController: VehicleController
    ...
    it('Should return SuccessRequest when exits vehicle', async () => {
      ...
      const resp = await vehicleController.findOne({ pathParameters: { id: '2' } } as any, {} as any, {} as any)

      expect(resp).toBeInstanceOf(SuccessRequest)
    })

    it('Should return NotFoundRequest when no exits vehicle', async () => {
      jest.spyOn(vehicleService, 'findOne').mockRejectedValue(new VehicleNotFound())
    ...
    it('Should return InternalServerError when fail vehicle service', async () => {
      jest.spyOn(vehicleService, 'findOne').mockRejectedValue(new Error('error'))
  ...
  describe('save', () => {
    it('Should return SuccessRequest when save successfully', async () => {
      ...
      const resp = await vehicleController.save({ body: JSON.stringify(newVehicleDto) } as any, {} as any, {} as any)
  ...
  describe('findAll', () => {
    it('Should return SuccessRequest when get data successfully', async () => {
      const resp = await vehicleController.findAll({} as any, {} as any, {} as any)

      expect(resp).toBeInstanceOf(SuccessRequest)
    })

    it('Should return InternalServerError when fail vehicle service', async () => {
      jest.spyOn(vehicleService, 'findAll').mockRejectedValue(new Error('error'))
